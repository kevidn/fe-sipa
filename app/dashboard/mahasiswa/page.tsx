...
export default function MahasiswaDashboard() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (userData.role.toUpperCase() !== 'MAHASISWA') {
        router.push('/dashboard'); // Atau ke halaman yang sesuai rolenya
        return;
      }
      setUser(userData);
    } catch (e) {
  ...
  const stats = [
    { label: 'Total Pengajuan', value: 12, icon: 'file-text', color: 'blue' },
    { label: 'Dalam Proses', value: 2, icon: 'clock', color: 'orange' },
    { label: 'Selesai', value: 9, icon: 'check-circle', color: 'green' },
    { label: 'Ditolak', value: 1, icon: 'x-circle', color: 'red' },
  ];
  ...
  const recentPengajuan: Pengajuan[] = [
    ...
    {
      id: '2',
      nomor: 'SKRIPSI-2024-015',
      jenis: 'Surat Izin Survei Penelitian (Skripsi)',
      tanggal: '2024-03-28',
      status: 'Diproses',
      sla: '2 Hari lagi',
      slaStatus: 'aman'
  ...
  if (loading) {
    return <div className="min-h-screen flex items-center justify-center bg-[#f4f3ee]">Memuat...</div>;
  }
  ...
  return (
    <>
      ...
      <section className="mb-10">
        <h2 className="text-4xl font-black mb-2 tracking-tight text-slate-800">Dashboard Mahasiswa</h2>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-[#00a86b] to-[#008f5d] rounded-[2.5rem] p-10 sm:p-14 text-white shadow-2xl shadow-[#00a86b]/20 mb-12 flex items-center justify-between">
        ...
        {/* Right side Icon - Match Screenshot */}
      ...
      <section className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden mb-12">
        <div className="p-8 sm:p-10 border-b border-slate-50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <button className="flex items-center justify-center gap-2 text-xs font-black text-slate-400 hover:text-sipa-green bg-slate-50 hover:bg-sipa-green/10 px-6 py-3.5 rounded-2xl transition-all duration-300 active:scale-95 w-full sm:w-auto uppercase tracking-widest">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="M21 2v6h-6M3 12a9 9 0 0 1 15-6.7L21 8M3 22v-6h6m12-4a9 9 0 0 1-15 6.7L3 16"/></svg>
            REFRESH
          </button>
        </div>
          ...
          <table className="w-full text-left min-w-[900px]">
            <thead>
              <tr className="text-xs font-black text-slate-400 uppercase tracking-widest border-b border-slate-50">
                ...
                <th className="px-8 py-6 text-center">SLA</th>
                <th className="px-8 py-6 text-right">Aksi</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50">
              {recentPengajuan.map((item, idx) => (
                <tr key={idx} className="group hover:bg-slate-50/50 transition-all duration-300">
                  <td className="px-8 py-8">
                    <span className="text-sm font-black text-slate-800 group-hover:text-sipa-green transition-colors">{item.nomor}</span>
                  </td>
                  <td className="px-8 py-8">
                    <p className="text-sm font-bold text-slate-600">{item.jenis}</p>
                  </td>
                  <td className="px-8 py-8 text-sm font-semibold text-slate-400">{item.tanggal}</td>
                  <td className="px-8 py-8">
                  ...
                  <td className="px-8 py-8">
                    <div className="flex justify-center">
                      <span className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest
                        ${item.slaStatus === 'tepat' ? 'bg-green-100 text-green-700' : ''}
                        ${item.slaStatus === 'aman' ? 'bg-blue-100 text-blue-700' : ''}
                        ${item.slaStatus === 'kurang' ? 'bg-amber-100 text-amber-700' : ''}
                      `}>
                        {item.sla}
                      </span>
                    </div>
        ...
        <div className="p-8 sm:p-10 bg-slate-50 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4">
          <span className="text-xs font-black text-slate-400 uppercase tracking-widest">Menampilkan 3 dari 12 pengajuan total</span>
