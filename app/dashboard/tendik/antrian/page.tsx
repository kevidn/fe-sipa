...
export default function AntrianBaru() {
  ...
  return (
    <div className="space-y-8 pb-10">
      {/* Breadcrumb */}
      <div>
        <Link href="/dashboard/tendik" className="text-emerald-600 font-bold text-sm flex items-center gap-1 hover:gap-2 transition-all">
      ...
      <div className="bg-white dark:bg-slate-900 p-6 rounded-[2rem] shadow-xl shadow-slate-200/50 dark:shadow-slate-950/50 border border-slate-100 dark:border-slate-800 flex flex-wrap gap-4 items-center transition-colors">
         ...
         <select 
            className="px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-800 border-none focus:ring-2 focus:ring-emerald-500/20 text-sm font-medium cursor-pointer dark:text-slate-200"
            value={filterJenis}
                     ...
                     filteredAntrian.map((item) => (
                        <tr key={item.id} className="hover:bg-slate-50/30 dark:hover:bg-slate-800/30 transition-all group">
                           ...
                           <td className="px-8 py-6 text-center">
                              <Link 
                                 href={`/dashboard/tendik/verifikasi/${item.id}`} 
